...
import { createSession, invalidateSession, invalidateAllSessions, verifyToken } from '../utils/jwt';
import { extractRequestMeta, getClientIp, generateFingerprint } from '../utils/geo';
...
async function verifyCaptcha(token: string): Promise<boolean> {
  ...
  if (!process.env.CAPTCHA_SECRET_KEY || !token) return true;
  try {
...
async function banIpAndFingerprint(ip: string, fingerprint: string): Promise<void> {
  ...
  logger.warn(`BAN automatique — IP: ${ip}`);
}

// ─── REGISTER ─────────────────────────────────────────────────────────
export async function register(req: Request, res: Response): Promise<void> {
  const { username, email, password, age, country, acceptRules, acceptPrivacy, captchaToken } = req.body;
  ...
  if (captchaToken) {
    ...
    if (!captchaValid) {
      res.status(400).json({ error: 'CAPTCHA invalide.' });
      return;
    }
  }
  ...
  res.status(201).json({ message: "Compte créé. Un administrateur doit valider votre inscription." });
}
...
export async function login(req: Request, res: Response): Promise<void> {
  ...
  const activeBan = await prisma.ban.findFirst({
    where: {
      AND: [
        { OR: [{ type: 'IP', value: meta.ip }, { type: 'FINGERPRINT', value: meta.fingerprint }] },
  ...
  if (activeBan) {
    res.status(403).json({ error: 'Accès refusé. Votre accès a été révoqué.', banned: true });
    return;
  }

  const failures = await countRecentFailures(meta.ip, meta.fingerprint);

  if (failures >= 2 && !captchaToken) {
    res.status(400).json({
  ...
  if (failures >= MAX_ATTEMPTS) {
    ...
    res.status(403).json({ error: 'Trop de tentatives. Accès bloqué temporairement.', banned: true });
    return;
  }

  const user = await prisma.user.findUnique({ where: { email } });
  const dummyHash = '$argon2id$v=19$m=65536,t=3,p=4$dummysaltdummysalt$dummyhash';
  const hashToVerify = user?.passwordHash || dummyHash;
  const passwordValid = await argon2.verify(hashToVerify, password).catch(() => false);

  if (!user || !passwordValid) {
    void prisma.loginAttempt.create({
      data: { ipAddress: meta.ip, fingerprint: meta.fingerprint, email, success: false, userAgent: meta.userAgent },
    }).catch(() => {});
  ...
  if (user.status === 'BANNED') {
    const banInfo = user.banExpiresAt ? `jusqu'au ${user.banExpiresAt.toLocaleString('fr-FR')}` : 'définitivement';
    res.status(403).json({ error: `Votre compte a été banni ${banInfo}.` });
    return;
  }

  const token = await createSession(user.id, meta.fingerprint, meta.ip, meta.userAgent);

  void prisma.loginAttempt.create({
    data: { ipAddress: meta.ip, fingerprint: meta.fingerprint, email, success: true, userAgent: meta.userAgent },
  }).catch(() => {});
  ...
  logger.info(`Connexion: ${user.username} depuis ${meta.ip} (${meta.country})`);
...
export async function logout(req: Request, res: Response): Promise<void> {
  if (req.sessionId) await invalidateSession(req.sessionId);
  void prisma.systemLog.create({
    data: { userId: req.user?.id, action: 'LOGOUT', ipAddress: req.clientIp, page: '/logout' },
  }).catch(() => {});
...
export async function logoutAll(req: Request, res: Response): Promise<void> {
  if (req.user) await invalidateAllSessions(req.user.id);
  res.clearCookie('token');
  res.json({ message: 'Toutes les sessions déconnectées.' });
}
...
export async function checkAuth(req: Request, res: Response): Promise<void> {
  ...
  if (!payload) { res.json({ authenticated: false }); return; }

  const user = await prisma.user.findUnique({
    where: { id: payload.userId },
    select: { id: true, username: true, email: true, role: true, status: true, rank: true, points: true, avatarUrl: true },
  });

  if (!user || user.status !== 'ACTIVE') { res.json({ authenticated: false }); return; }
