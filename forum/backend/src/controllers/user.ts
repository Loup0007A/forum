...
export async function editProfile(req: Request, res: Response): Promise<void> {
  ...
  await prisma.systemLog.create({
    data: { userId: req.user!.id, action: 'PROFILE_EDIT', ipAddress: req.clientIp },
  }).catch(() => {});
...
export async function sendMessage(req: Request, res: Response): Promise<void> {
  ...
  await prisma.systemLog.create({
    data: { userId: req.user!.id, action: 'MESSAGE_SEND', ipAddress: req.clientIp },
  }).catch(() => {});
...
export async function deleteAccount(req: Request, res: Response): Promise<void> {
  ...
  // Anonymize instead of deleting to keep thread integrity
...
export async function saveDraft(req: Request, res: Response): Promise<void> {
  const { content, title, threadId, forumId } = req.body;

  const draft = await prisma.draft.upsert({
    where: { id: req.body.draftId || 'nonexistent' },
    create: { content, title, userId: req.user!.id, threadId, forumId },
