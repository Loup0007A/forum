...
export async function getThread(req: Request, res: Response): Promise<void> {
  ...
  const thread = await prisma.thread.findUnique({
    where: { slug },
    include: {
      author: { select: { id: true, username: true, avatarUrl: true, rank: true, points: true, signature: true, createdAt: true, _count: { select: { posts: true } } } },
      forum: { include: { category: true } },
      tags: true,
      poll: { include: { options: { include: { _count: { select: { votes: true } } } }, votes: { where: { userId: req.user?.id } } } },
    },
  });
  ...
  // Increment views
  prisma.thread.update({ where: { id: thread.id }, data: { views: { increment: 1 } } }).catch(() => {});
...
export async function createThread(req: Request, res: Response): Promise<void> {
  ...
  await prisma.systemLog.create({
    data: {
      ...
      metadata: { threadId: thread.id, title },
    },
  });
...
export async function deleteThread(req: Request, res: Response): Promise<void> {
  ...
  const canDelete = req.user?.id === thread.authorId || 
...
export async function pinThread(req: Request, res: Response): Promise<void> {
  ...
  await prisma.adminLog.create({
    data: {
      ...
      targetId: id,
      targetType: 'Thread',
      metadata: { pinned: updated.isPinned },
    },
  });
...
export async function closeThread(req: Request, res: Response): Promise<void> {
  ...
  const updated = await prisma.thread.update({
    where: { id },
    data: { isLocked: !thread.isLocked, status: !thread.isLocked ? 'CLOSED' : 'OPEN' },
  });
  ...
  await prisma.adminLog.create({
    data: {
      ...
      targetId: id,
      targetType: 'Thread',
      metadata: { locked: updated.isLocked },
    },
  });
...
export async function searchThreads(req: Request, res: Response): Promise<void> {
  ...
  if (author) {
    where.author = { username: { contains: author as string, mode: 'insensitive' } };
  }
