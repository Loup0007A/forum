...
export async function sendRegistrationNotification(userData: {
  ...
  try {
    await transporter.sendMail({
      ...
      to: notifyEmail,
      subject: `🔔 Nouvelle inscription en attente — ${userData.username}`,
      html: `
        <div style="font-family: monospace; max-width: 600px; padding: 24px; background: #0f0f0f; color: #e0e0e0; border-radius: 8px;">
          ...
          <p style="margin-top: 24px; color: #a0a0a0;">Connectez-vous au panneau d'administration pour valider ou rejeter ce compte.</p>
        </div>
      `,
    });
